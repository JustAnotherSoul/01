...
updateRecord(H,Q) :- entry(H,Value,N,EF,Date), calculateInterval(N,EF,NewInterval), updateEF(EF,Q,NewEF), updatePractice(Date, NewInterval, NewDate), retract(entry(H,Value,N,EF,Date)), assertz(entry(H,Value,NewInterval,NewEF,NewDate)).
updateRecordFailure(H) :- entry(H, Value, N, EF, Date), updatePractice(Date, 1, NewDate), retract(entry(H,Value,N,EF,Date)), assertz(entry(H,Value,1,EF, NewDate)).
